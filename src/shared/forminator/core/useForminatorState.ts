...
import { useForminatorReducer } from './useForminatorReducer';

function isSetStateAction<S>(
...
export function useForminatorState<V>(): [
  V | null,
...
export function useForminatorState<V, Value>(
  ...
  const [value, dispatch] = useForminatorReducer(
    reducer,
    defaultValue,
    getDefaultValue,
  );
  ...
  const setFragmentValue = useCallback(
    (valueOrAction: SetStateAction<V | null>) => dispatch(valueOrAction),
    [dispatch],
  );
